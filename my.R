...
setwd("~/AnalyticsVidhya_LastManStanding-master")
...
# cross-validation
set.seed(786)
model_xgb <- xgb.cv(data=as.matrix(X_train), label=as.matrix(target), objective="multi:softprob", num_class=3, nfold=5, nrounds=550, eta=0.1, max_depth=6, subsample=0.9, colsample_bytree=0.9, min_child_weight=1, eval_metric="merror", prediction=T)
...
# model building
set.seed(786)
model_xgb <- xgboost(as.matrix(X_train), as.matrix(target), objective="multi:softprob", num_class=3, nrounds=550, eta=0.1, max_depth=6, subsample=0.9, colsample_bytree=0.9, min_child_weight=1, eval_metric='merror')
